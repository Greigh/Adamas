...
describe('Settings UI helpers', () => {
  ...
  test.skip('inserts collapse toggles into verbose/complex setting items', async () => {
    ...
    if (typeof settingsModule.addSettingCollapsibles === 'function') settingsModule.addSettingCollapsibles();

    const toggles = doc.querySelectorAll('#settings-view .settings-section .setting-item .collapse-toggle');
    // Debug: report counts to help identify why toggles may not be inserted
    const items = Array.from(doc.querySelectorAll('#settings-view .settings-section .setting-item'));
    const complex = items.filter((item) => {
      ...
      return textLength > 80 || hasFile || hasExport || controlCount > 1 || isDanger;
    });
    // log counts to help debugging when running tests
    // eslint-disable-next-line no-console
    console.log('items:', items.length, 'complexCandidates:', complex.length, 'toggles:', toggles.length);
    ...
    const sectionId = item.closest('.settings-section')?.id || item.closest('.settings-section')?.querySelector('h3')?.textContent?.trim().toLowerCase().replace(/\s+/g,'-');
