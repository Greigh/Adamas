...
describe('Gear buttons (open settings) - E2E', () => {
  ...
  test('hold timer gear button opens settings and scrolls to holdtimer section', async () => {
    ...
    const visible = await page.evaluate(() => {
      const el = document.querySelector('#settings-view [data-section="holdtimer"]');
      if (!el) return false;
      const rect = el.getBoundingClientRect();
      return rect.top >= 0 && rect.bottom <= (window.innerHeight || document.documentElement.clientHeight);
    });
  ...
  test('data management gear button opens settings and scrolls to data-management section', async () => {
    await page.goto(baseUrl, { waitUntil: 'domcontentloaded' });
    await page.waitForSelector('[data-section="data-management"] .open-settings-btn');
    await page.click('[data-section="data-management"] .open-settings-btn');
    await page.waitForSelector('#settings-view');
    await page.waitForSelector('#settings-view [data-section="data-management"]');
    const visible = await page.evaluate(() => {
      const el = document.querySelector('#settings-view [data-section="data-management"]');
      if (!el) return false;
      const rect = el.getBoundingClientRect();
      return rect.top >= 0 && rect.bottom <= (window.innerHeight || document.documentElement.clientHeight);
    });
