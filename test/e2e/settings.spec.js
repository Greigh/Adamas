...
describe('Settings E2E - float & toggle behavior', () => {
  ...
  test('floating a settings card creates a floating window and toggles the popout state', async () => {
    ...
    await page.evaluate(() => {
      ...
      window.openSectionInFloatingWindow = (sectionIdOrSelector) => {
        ...
        if (sectionIdOrSelector) {
          section = document.getElementById(sectionIdOrSelector) || document.querySelector(`[data-section="${sectionIdOrSelector}"]`) || document.querySelector(sectionIdOrSelector);
        }
    ...
    const sectionIdentifier = await page.$eval('.settings-section', el => el.id || el.dataset.section || '');
    await page.evaluate((id) => window.openSectionInFloatingWindow(id), sectionIdentifier);

    // Floating overlay should now contain a .floating-window
    await page.waitForSelector('#floating-overlay .floating-window', { timeout: 2000 });
    const floatingExists = await page.$eval('#floating-overlay .floating-window', el => !!el);
    expect(floatingExists).toBe(true);

    // The original card should have popped-out marker or popup button state
    const popped = await page.$eval('.settings-section', el => el.classList.contains('popped-out') || !!el.querySelector('.popped-out-indicator'));
    expect(popped).toBe(true);

    // Now minimize the floating window using its minimize control
    const minBtn = await page.$('#floating-overlay .floating-controls button[data-action="minimize"]');
    if (minBtn) {
      await minBtn.click();
  ...
  test('minimize toggles in settings header work', async () => {
    ...
    const display = await page.$eval('.settings-section .section-content', el => window.getComputedStyle(el).display);
