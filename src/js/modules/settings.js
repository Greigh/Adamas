...
import { saveData, loadData, loadPatterns, savePatterns, loadSteps, saveSteps, loadNotes, saveNotes } from './storage.js';
...
export function applySettings() {
    ...
    Object.entries(toggles).forEach(([toggleId, section]) => {
        ...
        if (toggle && sectionEl) {
            toggle.checked = appSettings[`show${section.charAt(0).toUpperCase() + section.slice(1)}`];
            sectionEl.style.display = toggle.checked ? '' : 'none';
        }
    });
    
    // Apply export settings
    const exportSettings = ['export-patterns', 'export-steps', 'export-notes', 'export-settings'];
    ...
    // Add the new setting
    const timerAllowHistoryDeletion = document.getElementById('timer-allow-history-deletion');
    if (timerAllowHistoryDeletion) timerAllowHistoryDeletion.checked = appSettings.timerAllowHistoryDeletion;
...
export function setupSettingsEventListeners() {
    ...
    // Settings toggles
    const settingsToggles = ['toggle-formatter', 'toggle-callflow', 'toggle-notes', 'toggle-holdtimer'];
    settingsToggles.forEach(toggleId => {
        const toggle = document.getElementById(toggleId);
        if (toggle) {
            toggle.addEventListener('change', function() {
                const section = toggleId.replace('toggle-', '');
                appSettings[`show${section.charAt(0).toUpperCase() + section.slice(1)}`] = this.checked;
    ...
    // Export toggles
    const exportToggles = ['export-patterns', 'export-steps', 'export-notes', 'export-settings'];
    ...
    // Timer settings
    const timerToggles = ['timer-auto-start', 'timer-sound-alerts', 'timer-show-notifications', 'timer-log-holds', 'timer-countdown-mode'];
    ...
    const timerWarningInput = document.getElementById('timer-warning-time');
    const timerWarningValue = document.getElementById('timer-warning-time-value');
    const timerWarningSlider = document.getElementById('timer-warning-time-slider');
    ...
    if (alertSound) {
        ...
        if (customSoundOption) {
            customSoundOption.style.display = alertSound.value === 'custom' ? '' : 'none';
        }
...
function exportData() {
    ...
    a.href = url;
    a.download = `call-center-helper-data-${new Date().toISOString().split('T')[0]}.json`;
...
function resetLayout() {
    if (confirm('Reset layout to default? This will restore the original section order and grid settings.')) {
        ...
        if (gridColumns) {
            ...
            document.getElementById('grid-columns-value').textContent = appSettings.defaultLayout.columns;
        }
        if (gridSpacing) {
            gridSpacing.value = appSettings.defaultLayout.spacing;
            document.getElementById('grid-spacing-value').textContent = appSettings.defaultLayout.spacing + 'px';
        }
    }
}
...
export async function lazyLoadDynamicComponent(componentId, moduleType) {
  ...
  switch (moduleType) {
    case 'timer':
      const { initializeTimer, setupTimerEventListeners } = await import('./timer.js');
