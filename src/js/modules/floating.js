export class FloatingWindowManager {
    ...
    floatSection(sectionId, forceBrowserWindow = false) {
        ...
        const shouldOpenInBrowser = forceBrowserWindow || appSettings?.enablePopupWindows;
    ...
    openInBrowserWindow(sectionId) {
        ...
        tempDiv.querySelectorAll('[id]').forEach(element => {
            ...
            tempDiv.querySelectorAll(`label[for="${originalId}"]`).forEach(label => {
                label.setAttribute('for', newId);
            });
        });
        ...
        console.log('🌐 [DEBUG] Opening browser window with features:', windowFeatures);
        ...
        const newWindow = window.open('about:blank', `${sectionId}-popup-${Date.now()}`, windowFeatures);
        ...
        if (newWindow) {
            ...
            console.warn('Failed to open browser window - popup blocked or not supported');
    ...
    addPoppedOutIndicator(sectionId, text = 'Popped Out') {
        ...
        if (!section.querySelector('.popped-out-indicator')) {
            ...
            const header = section.querySelector('.section-header .title-container') || 
