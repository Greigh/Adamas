...
function createTemplatesUI() {
  ...
  // Remove existing if present (to avoid duplicates on re-init)
  const existingContainer = document.querySelector('.call-templates-container');
  ...
  templatesContainer.innerHTML = `
    <div class="templates-header">
      <div class="template-controls-group">
        <label for="call-template-select">Call Template:</label>
        <div class="template-input-wrapper">
            <select id="call-template-select" class="template-select" style="flex:1;">
...
function updateTemplateSelector() {
  ...
  Object.values(callTemplates).forEach(template => {
  ...
  if (currentValue && callTemplates[currentValue]) {
...
function setupTemplateListeners() {
  ...
  if (templateSelect) {
    templateSelect.addEventListener('change', (e) => {
      ...
      if (selectedId && callTemplates[selectedId]) {
        const template = callTemplates[selectedId];
  ...
  if (applyBtn) {
    applyBtn.addEventListener('click', () => {
      ...
      if (selectedId && callTemplates[selectedId]) {
        applyCallTemplate(callTemplates[selectedId]);
        showToast(`Applied "${callTemplates[selectedId].name}" template`, 'success');
      }
    });
  }
...
const DEFAULT_FIELDS = [
  { id: 'priority', label: 'Priority', type: 'select', options: ['Low', 'Medium', 'High', 'Critical'], visible: false, system: true },
  { id: 'category', label: 'Category', type: 'text', placeholder: 'e.g. Billing, Tech Support', visible: false, system: true },
  { id: 'followUp', label: 'Follow-up Required', type: 'checkbox', visible: false, system: true }
];
...
function loadSettings() {
  ...
  if (stored) {
    ...
    if (parsed.showPriority !== undefined || parsed.showCategory !== undefined) {
...
export function applyCallLogSettings() {
  ...
  callLogSettings.fields.forEach(field => {
      ...
      if (field.type === 'select') {
          ...
          div.innerHTML = `
            <label for="${fieldName}">${field.label}</label>
            <input type="text" id="${fieldName}" name="${fieldName}" class="form-control" placeholder="${field.placeholder || ''}">
          `;
      } else if (field.type === 'textarea') {
          div.innerHTML = `
            <label for="${fieldName}">${field.label}</label>
            <textarea id="${fieldName}" name="${fieldName}" class="form-control" rows="3" placeholder="${field.placeholder || ''}"></textarea>
          `;
      ...
      // Insertion Logic:
      // Hardcoded preference for system fields to try and stay in that "second row" if possible?
      // Or just stack them all before the controls for simplicity and robustness.
...
export function showCallLoggingSettings() {
    ...
    <div class="modal call-settings-modal" style="max-width: 900px; width: 95%;">
      ...
      <div class="modal-body">
        <div class="tabs tab-header" style="margin-bottom: 20px;">
            <button class="tab-btn tab-button active" data-tab="templates">Templates</button>
            <button class="tab-btn tab-button" data-tab="form-settings">Form Settings</button>
        </div>
        ...
        <!-- Tab 2: Form Settings -->
        <div id="tab-form-settings" class="tab-content" style="display: none;">
  ...
  typeSelect.addEventListener('change', () => {
      ...
      placeholderGroup.style.display = (type === 'text' || type === 'textarea') ? 'block' : 'none';
  });
  ...
  function renderFieldList() {
      ...
      callLogSettings.fields.forEach(field => {
          ...
          li.style.cssText = 'padding: 10px; border-bottom: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items: center; cursor: pointer;';
  ...
  function loadFieldForm(field) {
      ...
      modal.querySelector('#edit-field-system').value = field.system ? 'true' : 'false';
      ...
      modal.querySelector('#edit-field-placeholder').value = field.placeholder || '';
      modal.querySelector('#edit-field-options').value = (field.options || []).join(', ');
      ...
      Array.from(fieldListEl.children).forEach(child => {
        child.style.backgroundColor = child.textContent.includes(field.label) ? 'var(--primary-blue-light, #eff6ff)' : '';
      });
  }
  ...
  fieldForm.addEventListener('submit', (e) => {
      ...
      if (type === 'text' || type === 'textarea') fieldData.placeholder = placeholder;

      // Update or Add
      const existingIndex = callLogSettings.fields.findIndex(f => f.id === fieldData.id);
      if (existingIndex >= 0) {
          callLogSettings.fields[existingIndex] = { ...callLogSettings.fields[existingIndex], ...fieldData };
      } else {
  ...
  deleteFieldBtn.addEventListener('click', async () => {
      ...
      if (isSystem) {
          showToast('Cannot delete system fields. Try hiding them instead.', 'warning');
          return;
      }

      const confirmed = await showConfirmModal({
          title: 'Delete Field',
          message: 'Are you sure you want to delete this field? Data in this field for previous calls will not be lost, but it will disappear from the form.',
          confirmLabel: 'Delete',
          danger: true
      });

      if (confirmed) {
          callLogSettings.fields = callLogSettings.fields.filter(f => f.id !== id);
          saveSettings();
          renderFieldList();
  ...
  const refreshList = () => {
    ...
    Object.values(callTemplates).forEach(t => {
        const li = document.createElement('li');
        li.style.cssText = 'padding: 10px; border-bottom: 1px solid var(--border-color); cursor: pointer; border-radius: 4px;';
  ...
  const loadForm = (template) => {
    ...
    modal.querySelector('#edit-template-desc').value = template.description || '';
    modal.querySelector('#edit-template-type').value = template.fields.callType || 'inbound';
    modal.querySelector('#edit-template-notes').value = template.fields.initialNotes || '';
    
    Array.from(listEl.children).forEach(child => {
        child.style.backgroundColor = child.textContent === template.name ? 'var(--primary-blue-light, #eff6ff)' : '';
    });
  };
  ...
  formEl.addEventListener('submit', (e) => {
    e.preventDefault();
    const id = modal.querySelector('#edit-template-id').value || 'tpl_' + Date.now();
